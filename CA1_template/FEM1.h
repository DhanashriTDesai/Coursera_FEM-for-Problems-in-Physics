...
double FEM<dim>::basis_function(unsigned int node, double xi){
  ...
  //EDIT
	...
	if(basisFunctionOrder==1){
		...
		else if(node==2) //xi=-1/3
			value=27./16.*(xi+1.)*(xi-1./3.)*(xi-1.);
		else if(node==3) //xi=1/3
			value=-27./16.*(xi+1.)*(xi+1./3.)*(xi-1.);
	}
...
double FEM<dim>::basis_gradient(unsigned int node, double xi){
  ...
  //EDIT
	...
	if(basisFunctionOrder==1){
		...
		if(node==0) //xi=-1
			value=-9./16.*(3.*xi*xi-2.*xi-1./9.);
		else if(node==1) //xi=1
			value=9./16.*(3.*xi*xi+2.*xi-1./9.);
		else if(node==2) //xi=-1/3
			value=27./16.*(3.*xi*xi-2./3.*xi-1);
		else if(node==3) //xi=1/3
			value=-27./16.*(3.*xi*xi+2./3.*xi-1);
	}
...
void FEM<dim>::assemble_system(){
  ...
  for (;elem!=endc; ++elem){
    ...
    //Loop over local DOFs and quadrature points to populate Flocal i.e. element force vector
    Flocal = 0.;  
	...
	f = pow(10,11)*x;
	//EDIT - Define Flocal.
	Flocal[A] += f * Area * h_e / 2. * basis_function(A,quad_points[q]) * quad_weight[q];
      }
    }
	...
	Flocal[1] = pow(10,6); // EAu.x at x=L = h = 10^6
      }
    }

    //Loop over local DOFs and quadrature points to populate Klocal i.e. element stiffness matrix
    Klocal = 0; YM = pow(10,11);
    for(unsigned int A=0; A<dofs_per_elem; A++){
      for(unsigned int B=0; B<dofs_per_elem; B++){
	for(unsigned int q=0; q<quadRule; q++){
	  //EDIT - Define Klocal.
		Klocal[A][B] += 2. * YM * Area / h_e * basis_function(A,quad_points[q]) * basis_function(B,quad_points[q]) * quad_weight[q];
	}
      }
    }
  ...
  MatrixTools::apply_boundary_values (boundary_values, K, D, F);
}
...
double FEM<dim>::l2norm_of_error(){
  ...
  for (;elem!=endc; ++elem){
	...
	u_h += D[local_dof_indices[B]]*basis_function(B,quad_points[q]);
      ...
      if(prob==1){
	u_exact = -pow(x,3)/6.0 + 0.0117*x;
      }
      else{
	u_exact = -pow(x,3)/6.0 + 0.105*x;
      }
